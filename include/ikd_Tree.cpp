...
void KD_TREE<PointType>::Build(PointVector point_cloud) {
    ...
    // CHECK_CUDA_ERROR(cudaMallocManaged((void **)STATIC_ROOT_NODE, sizeof(KD_TREE_NODE)));
    cudaMallocManaged((void **)&STATIC_ROOT_NODE, sizeof(KD_TREE_NODE));
...
void KD_TREE<PointType>::Build(PointType *point_cloud, size_t size) {
    ...
    if (size == 0) return;
    // CHECK_CUDA_ERROR(cudaMallocManaged((void **)STATIC_ROOT_NODE, sizeof(KD_TREE_NODE)));
    if (cudaMallocManaged((void **)&STATIC_ROOT_NODE, sizeof(KD_TREE_NODE)) != cudaSuccess) {
        std::cout << "could not alloc root node" << std::endl;
        return;
    }
...
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage) {
    if (l > r) return;
    // CHECK_CUDA_ERROR(cudaMallocManaged((void **)root, sizeof(KD_TREE_NODE)));
    cudaMallocManaged((void **)root, sizeof(KD_TREE_NODE));
    InitTreeNode(*root);
    int mid = (l + r) >> 1;
    int div_axis = 0;
    int i;
